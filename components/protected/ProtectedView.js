...
const ProtectedView = () => {
  ...
  const getSemsSetter = (sem, courseIndex, field) => (newVal) => {
    setSems((prev) => {
      const _sems = { ...prev };
  ...
  const handleSubmit = async (event) => {
    ...
    setQrLink(`https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(data)}&size=1000x1000`);
      ...
      <div className="flex flex-col w-full max-w-[1200px] items-center">
        {isLoading ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 items-center py-2">
              <div className="flex gap-4 items-center">
                <FormInput field="Student name" value={studentName} setter={setStudentName} />
                <FormInput field="Student id" value={studentId} setter={setStudentId} />
              </div>
              <div className="flex gap-4 items-center justify-center">
                <FormInput field="Institution name" value={institutionName} setter={setInstitutionName} />
                <FormInput field="Institution id" value={institutionId} setter={setInstitutionId} />
              </div>
                  ...
                  <>
                    ...
                    <div className="flex flex-col items-end" key={`div-${sem}`}>
                      {sems[sem].map((_course, courseIndex) => (
                        <div className="flex gap-4 self-stretch py-2" key={courseIndex}>
                          <FormInput
                            field="Course ID"
